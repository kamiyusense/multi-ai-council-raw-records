...
class Runner:
    ...
    def execute(self, payload, task_id, seq, target_thread):
        ...
        # Approval fail-closed (Wait for human, no auto-accept/deny)
        ...
        # Check current state for retries
        try:
        ...
        if not res_ok:
            ...
            if res_detail == "DATABASE_LOCKED":
                 return "BLOCKED", "CONCURRENT_RESERVATION_FAILED"
        ...
        if self.transport_mode == "mock":
            ...
            self.transport = PrepareLiveTransport()
            self.ledger.update_state(req_id, "PREPARED")
            return "PREPARED", "PREPARE_LIVE_SUCCESS"
        elif self.transport_mode == "live":
             self.transport = LiveAppServerTransport(self.config)
        else:
            self.ledger.update_state(req_id, "BLOCKED", error_code="INVALID_TRANSPORT")
            return "BLOCKED", "INVALID_TRANSPORT"

        # Ensure we actually start the transport before sending.
        try:
             self.transport.start()
        except Exception as e:
             if str(e) == "Live execution is STRICTLY PROHIBITED in this POC.":
                  # Live execution block caught as expected
                  self.ledger.update_state(req_id, "BLOCKED", error_code="LIVE_BLOCKED_DURING_START")
                  return "BLOCKED", "LIVE_BLOCKED_DURING_START"
             raise e
        ...
        try:
            res = self.transport.send(payload)
        except Exception as e:
